...
def _ffmpeg_cmd_with_quality(url: str, fps: int = 10, jpeg_quality: int = 5) -> list[str]:
...
async def _ffmpeg_mjpeg_generator(
    ...
    while True:
        proc = await asyncio.create_subprocess_exec(
            *_ffmpeg_cmd_with_quality(url, fps=fps, jpeg_quality=jpeg_quality),
...
async def mjpeg_generator(
    ...
    if url.lower().startswith("rtsp://"):
        async for frame in _ffmpeg_mjpeg_generator(url, fps, BOUNDARY, jpeg_quality=jpeg_quality):
            yield frame
        return
